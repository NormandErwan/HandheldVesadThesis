...
Nous avons donc réalisé notre propre prototype de visiocasque \reffigureETSp{ARRift.jpg} similaire à l'AR-Rift \citep{Steptoe2013} : c'est un visiocasque de RA vidéo de conception simple qui a fait ses preuves, utilisé par \cite{Steptoe2014} et \cite{Piumsomboon2014}. Il consiste à coller une caméra stéréoscopique (avec deux objectifs capturant deux images à la fois, une pour chaque \oe il) à un visiocasque de RV, puis fonctionne sur le principe suivant :
\begin{enumerate}
  \item La caméra stéréoscopique filme l'environnement réel de l'utilisateur \reffigureETSp{OvrvisionMarker_1.jpg}.
  \item Un logiciel va corriger les déformation éventuelles des deux images de cette caméra (rectification) \reffigureETSp{OvrvisionMarker_2.jpg}.
  \item Un moteur 3D va ajouter du contenu virtuel sur les deux images \reffigureETSp{OvrvisionMarker_3.jpg}.
...
\figureLayoutETS{OvrvisionMarker}{%
  \subfigureETS[0.2]{OvrvisionMarker_1.jpg}{Vue non rectifiée : l'image présente des déformations importantes.}%
  \figurehspace%
  \subfigureETS[0.2]{OvrvisionMarker_2.jpg}{Vue rectifiée : les lignes sont de nouveau droites (par exemple le clavier).}%
  \figurehspace%
  \subfigureETS[0.2]{OvrvisionMarker_3.jpg}{Vue rectifiée et augmentée : un cube 3D transparent rouge a été ajouté par dessus un marqueur.}%
}{
  Vue de l'objectif gauche de l'Ovrvision Pro à travers les différentes étapes de la capture de l'image, sa rectification et son augmentation avant de pouvoir l'afficher dans le visiocasque.
}

Enfin, nous pourrions simplifier le visiocasque en n'utilisant une caméra monoscopique (avec un seul objectif capturant une seule image à la fois), et afficher l'image capturée aux deux yeux. Cependant, comme le souligne \cite{Bourke1999}, la vision stéréoscopique est l'un des principal indice utilisé par le cerveau pour percevoir la profondeur : les yeux étant séparés horizontalement par une distance appelée écart pupillaire, chaque \oe il perçoit une image légèrement différente \reffigureETSp{Ovrvision} permettant au cerveau de percevoir en 3D. C'est pourquoi nous choisissons d'utiliser une caméra stéréoscopique.
...
Une seconde limite évidente est la portabilité de notre visiocasque.
...
Nous avons donc besoin en premier lieu d'une caméra stéréoscopique \texten{fisheye} pour capturer les images de l'environnement réel et d'un visiocasque de RV pour afficher les images augmentées à l'utilisateur. Ils doivent tout deux 
