...
\begin{itemize}
  ...
  \item \code{IArucoObjectsTracker} et \code{ArucoObject} détectent les marqueurs sur les images et place les éléments virtuels.
  \item \code{IArucoCameraDisplay} configure la caméra virtuelle.
    ...
    \begin{umlseqdiag}
      ...
      \begin{umlcall}[op={ImagesUpdated()}]{ArucoCamera}{ArucoObjectsTracker}
        \begin{umlfragment}[type=par, inner xsep=3]
          ...
          \end{umlcallself}
          \begin{umlcallself}[op={UpdateTransforms()}]{ArucoObjectsTracker}
          \end{umlcallself}
          \umlfpart[thread]
...
\begin{enumerate}
  \item On ouvre la scène \code{Assets/ArucoUnity/Scenes/TrakcMarkers.unity} préparée pour le suivi de marqueurs avec une caméra monoscopique rectilinéaire.
...
\begin{listingETS}{cs}{lst:aruco_camera_display}{Classe C\# MonoArucoCameraDisplay simplifiée.}
  public class MonoArucoCameraDisplay : ArucoCameraDisplay
  {
    ...
    protected void Start()
    {
      ...
      bg.transform.parent = this.transform.parent;
...
\begin{listingETS}{cs}{lst:aruco_camera_display}{Classe C\# ArucoObjectsTracker simplifiée.}
  public class ArucoObjectsTracker : ArucoObjectsController, IArucoObjectsTracker
  {
    public List<ArucoObject> arucoObjects;
    public ArucoCameraDisplay display;
    public DetectorParameters detectParams;
...
Notre caméra virtuelle étant désormais prête pour la RA, nous pouvons utiliser le suivi de marqueur pour savoir où placer nos éléments virtuels.
