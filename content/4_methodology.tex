...
\figureLayoutETS{PinholeCameraCalibration}{%
  \subfigureETS[0.4]{PinholeCameraCalibration_1.jpg}{Sélection de la première webcam disponible (identifiant \code{0} dans le champs \code{WebcamId}).}%
  \figurehspace%
...
\begin{enumerate}
  ...
  \item Dans l'objet \code{ArucoWebcam}, sur le script \code{ArucoCameraParametersController}, on renseigne le nom du fichier d'étalonnage dans le champs \code{CameraParametersFilename}.
...
\begin{enumerate}
  ...
  \item En jouant la scène, les marqueurs virtuels sont placés aux même positions et orientations que les marqueurs physiques détectés \reffigureETSp{TrackMarkersDetection}.
\end{enumerate}
...
\figureLayoutETS{OvrvisionCalibration}{%
  ...
  Configuration de l'étalonner dans Unity de la caméra Ovrvision.
}
...
Pour l'étalonnage, la procédure à suivre est la même que dans la \autoref{subsec:pinhole_camera_calibration}. Nous désactivons cependant l'objet \code{PinholeCameraCalibration} pour travailler avec les objets \code{Assets/ArucoUnity/Prefabs/Cameras/OvrvisionArucoCamera} pour la caméra et \code{Assets/ArucoUnity/Prefabs/Calibration/StereoOmnidirCameraCalibration} que nous glissons dans la scène \reffigureETSp{OvrvisionCalibration}. Nous avons donc créée un nouveau script d'étalonnage, \code{StereoOmnidirCameraCalibration} : très similaire à \autoref{lst:pinhole_camera_calibration}, nous avons seulement remplacé la fonction d'étalonnage par \code{Cv.Omnidir.StereoCalibrate}. Cette dernière étalonne chaque caméra individuellement, puis calcule la position et l'orientation relative des deux plans images, et retourne tous ces paramètres que l'on enregistre dans un fichier d'étalonnage.

\figureETS[0.9]{OvrvisionTracking.jpg}{%
...
On met finalement en place la RA dans notre visiocasque. Comme pour l'étalonnage, la procédure est la même que dans les sous-sections \ref{subsec:pinhole_camera_undistortion} et \ref{subsec:pinhole_camera_display}, mais on utilise des objets différents : on désactive donc les objets de correction et d'affichage pour la webcam et on glisse dans la scène ceux pour une caméra stéréoscopique \texten{fisheye} ainsi que \code{OvrvisionArucoCamera} \reffigureETSp{OvrvisionTracking.jpg}. Contrairement à la correction des caméras rectilinéaires, les fonctions du module omnidir nous permettent de choisir directement le champs de vision des caméras gauches et droites corrigées. On les définis toutes les deux à un champs de vision vertical de \SI{105}{\deg}, comme le visiocasque de RV que nous nous utilisons. Les marqueurs et leur suivi une fois configurés, notre visiocasque fonctionne comme nous l'avons décrit à la \autoref{subsec:prototype_operation} !
...
\begin{listingETS}{cs}{lst:stereo_vr_aruco_camera_display}{Classe C\# StereoVRArucoCameraDisplay simplifiée.}
  public class StereoVRArucoCameraDisplay : ArucoCameraDisplay
  {
    protected override void Start()
    {
      cameraFov[0] = Undistort.Fovs[0], cameraFov[1] = Undistort.Fovs[1]; // (1)
      base.Start();

      Cameras[0].stereoTargetEye = StereoTargetEyeMask.Left; // (2)
      Cameras[1].stereoTargetEye = StereoTargetEyeMask.Right;

      Cameras[1].transform.localPosition += Undistort.CameraParameters.T; // (3)
    }
  }
\end{listingETS}
