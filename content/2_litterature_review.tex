...
Dépasser les limites physiques d'un écran est un thème courant dans la littérature. En effet, un petit écran comme ceux des téléphones intelligents, ou des PCs dans les années 2000 \citep{Baudisch2002}, ne permet pas de visualiser et de naviguer correctement de grands documents : il n'est pas possible de visualiser en même temps les détails et une vue d'ensemble du document \reffigureETSp{Guiard2004_1.png}.

\figureETS{Guiard2004_1.png}{
  Représentations de la visualisation d'un large document sur un écran plus petit. À gauche : la vue de l'écran sur le document représentée par un rectangle noir. Au milieu : le document représenté à différents niveaux d'échelle, le long de l'axe S (\texten{scale}), la vue restant de taille constante. À droite : vues du document à différentes échelles.\\
...
\begin{itemize}
  \item \texten{Pan+Zoom} : une vue peut-être déplacée sur le document par défilement (\texten{pan}), tandis que le zoom permet de changer l'échelle du document (\reffigureETS{Guiard2004_2.png}, gauche).
  \item \texten{Overview+Detail} : une vue zoomée (\texten{detail}) et une vue sur le document en entier (\texten{overview}) sont affichées simultanément (\reffigureETS{Guiard2004_2.png}, milieu). Elles sont placées côte-à-côte ou superposées et peuvent être déplacées indépendamment. Cependant, la navigation peut-être difficile si la différence d'échelle est trop grande entre les deux vues.
  \item \texten{Focus+Context} : une seule vue permet d'afficher le document en entier et le document zoomé, grâce à une distortion optique (\reffigureETS{Guiard2004_2.png}, droite). Aucune partie du document n'est cachée, mais il peut être difficile d'interpréter le contenu de la vue à cause de la distortion.
\end{itemize}

\figureETS{Guiard2004_2.png}{
  Représentations de techniques de visualisation et de navigation d'un large document sur un écran plus petit. À gauche : la technique \texten{Pan+Zoom} permet de défiler et zoomer à travers une seule vue dans le document. Au milieu : la technique \texten{Overview+Detail} affiche simultanément une vue zoomée et une vue du document en entier. À droite : la technique \texten{Focus+Context} affiche vue du document en entier et un zoom par une distortion optique (sphérique à gauche et linéaire à droite).\\
