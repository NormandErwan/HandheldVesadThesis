...
\figureLayoutETS{Billinghurst2005}{%
  \begin{tikzpicture}[font=\footnotesize]
    ...
    \draw[->, very thick] (1.5,-0.5) -- (3.5,-0.5)
      node[label, pos=0.5, below]{Technique\\d'interaction};
...
\figureETS[0.5]{Jacob2008.jpg}{
  Illustrations des deuxième et troisième générations d'IHM, en ligne de commande puis par interfaces graphiques WIMP, ainsi que les nombreux types émergeants d'IHMs \textquote{post-WIMP}. Toutes sont complémentaires, chacune adaptée à des usages spécifiques qu'étudie la recherche sur les IHMs.\\
  Tiré de \cite{Jacob2008}.
}

Si de nombreuses IHMs \textquote{post-WIMP} ont par la suite été développées, comme celles désormais bien connues des téléphones intelligents à écrans tactiles \cite{Jacob2008}, aucun paradigme pour la RA n'est encore établi \citep{VanKrevelen2010}. \cite{Billinghurst2005} décrit le processus du développement d'un paradigme d'IHM pour un nouveau média :
\begin{itemize}
...
Pour \cite{Billinghurst2015} toutes les IHMs qui ont été développées pour la RA atteignent seulement les première et deuxième étapes, important et appliquant des techniques d'interactions des IHMs WIMP, de la RV ou utilisées avec des écrans tactiles. Ils les classent en plusieurs catégories :
\begin{enumerate}
  ...
  \item Interfaces utilisateur tangibles (IUT) : utilisant des objets réels pour interagir avec du contenu virtuel.
  \item Interfaces utilisateur naturelles : utilisant les mouvements du corps de l'utilisateur, par exemple des commandes gestuelles.
  \item Interfaces multimodales : utilisant simultanément des commandes vocales et gestuelles.
\end{enumerate}

Nous détaillons les interfaces en 3D, les interfaces naturelles et les IUT car ce sont celles qui nous concernent pour notre téléphone à écran étendu.

\subsection{Interfaces en 3D et interfaces naturelles}
\label{subsec:litterature_ar_hci_interactions}
Ces interfaces et interactions sont importées des recherches dans les environnements en 3D (affichés sur un écran d'ordinateur) et de la RV. \cite{Bowman2004} les classent en trois catégories : (1) navigation, (2) sélection et (3) manipulation. Seules ces deux dernières sont applicables à la RA, le contenu virtuel étant aligné avec l'environnement réel, l'utilisateur se déplace simplement lui-même.

De nombreux dispositifs ont été développés pour les IHMs 3D : des souris 3D, des joysticks, des stylos suivis en 3D ou des dispositifs haptiques (avec retour de force, donnant l'illusion de toucher le contenu virtuel) \reffigureETSp{Adcock2003.jpg}. Tous permettent de manipuler (translation et rotation) avec précision du contenu virtuel, mais présentent souvent l'inconvénient d'occuper les mains de l'utilisateur : les espaces de visualisation et d'interaction sont donc séparés, mais, surtout, les méthodes d'interactions avec les objets virtuels sont alors différentes de celles avec les objets réels \cite{Billinghurst2015}.

\figureETS[0.6]{Adcock2003.jpg}{
  Exemple d'une IHM de RA utilisant des dispositifs d'IHMs d'environnements en 3D : la main gauche utilise plusieurs boutons pour interagir avec le menu 2D, tandis que la main droite tient un stylo haptique pour manipuler le crâne avec 6 DoF.\\
  Tiré de \cite{Adcock2003}.
}

Les interfaces utilisateurs naturelles utilisent directement les mouvements du corps de l'utilisateur comme dispotif d'interaction, en particulier les mains. De nombreuses méthodes de vision par ordinateur permettent ce suivi en utilisant simplement les images d'une caméra \cite{Billinghurst2015}. Un utilisateur peut alors agir directement avec les objets virtuels comme il le ferait avec des objets physiques, ou exécuter des commandes avec des gestes avec ses mains.
...
La séparation entre les styles d'IHMs présentées \cite{Rekimoto1995} \reffigureETSp{Rekimoto1995.png} est artificielle : comme nous l'avons vu, l'échelle de \cite{Milgram1994} nous montre que la RA devrait s'intégrer avec l'informatique ubiquitaire, les GUI et la RV \cite{Billinghurst2005}. Par exemple, \cite{Heun2016} utilise la RA comme interface pour interagir de façon naturelle avec les objets intelligents autour de soi \reffigureETSp{Heun2016.jpg}, tandis que le prototype \texten{SpaceTop} de \cite{Lee2013} transforme un GUI sur ordinateur en 3D \reffigureETSp{Lee2013.jpg}.
