...
\figureLayoutETS{DesignConsequences}{%
  ...
  \figurehspace%
  \subfigureETS{HandheldVESADApps3.jpg}{
    Redesign of \reffigureETS{HandheldVESADMap.jpg} based on the zones in \reffigureETS{HandheldVESADZones.png} (mock-up).
  }%
}{
}

We used the layout guide in \reffigureETS{HandheldVESADZones.png} to redesign one of our earlier mock-ups \reffigureETSp{HandheldVESADApps3.jpg}. As on the original mock-up, the hand is rotating the phone $\approx$\ang{90} to access alternative screens, which now show up on the side of the user's right (dominant) hand to facilitate selection of an alternative screen. Notifications appear above the screen, since they are for output only.
