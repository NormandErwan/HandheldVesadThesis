...
Enfin, on peut mettre en \oe uvre des techniques de visualisation comme Overview+Detail ou Focus+Context sur l'écran étendu. La vue détaillée peut être affichée sur l'écran physique ou sur la fenêtre virtuelle. Par exemple, la \reffigureETS{HandheldVESADDogs} montre une image détaillé dans la fenêtre virtuelle (\texten{detail}) sélectionnée depuis une galerie d'images affichée sur l'écran physique (\texten{overview}). Ces deux vues peuvent donc être inversées, par exemple pour profiter de la haute définition de l'écran physique.
